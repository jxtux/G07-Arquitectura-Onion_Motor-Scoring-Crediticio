...
public record Dinero(BigDecimal monto,Moneda moneda){
    public Dinero{
        ...
        if(monto.signum()<0)throw new DomainException("El monto no puede ser negativo.");
    }
    public static Dinero positivo(BigDecimal monto,Moneda moneda){
        Dinero d=new Dinero(monto,moneda);
        if(d.monto.signum()<=0)throw new DomainException("El monto debe ser mayor que cero.");
        return d;
    }
}
