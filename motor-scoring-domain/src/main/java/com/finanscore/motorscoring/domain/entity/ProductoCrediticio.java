...
public final class ProductoCrediticio{
    ...
    public ProductoCrediticio(Long id,String codigo,String nombre,Dinero min,Dinero max,int pmin,int pmax,EstadoProducto estado,Long modelo){
        ...
        if(min.moneda()!=max.moneda()||min.monto().compareTo(max.monto())>0)throw new IllegalArgumentException("Rango monetario inválido.");
        if(pmin<=0||pmax<pmin)throw new IllegalArgumentException("Rango de plazo inválido.");
    ...
    public void validarSolicitud(Dinero monto,int plazo){
        if(estado!=EstadoProducto.ACTIVO)throw new SolicitudNoEvaluableException("El producto está inactivo.");
        if(monto.moneda()!=montoMinimo.moneda())throw new SolicitudNoEvaluableException("La moneda no corresponde al producto.");
        if(monto.monto().compareTo(montoMinimo.monto())<0||monto.monto().compareTo(montoMaximo.monto())>0)throw new SolicitudNoEvaluableException("El monto está fuera de los límites del producto.");
        if(plazo<plazoMinimo||plazo>plazoMaximo)throw new SolicitudNoEvaluableException("El plazo está fuera de los límites del producto.");
    }
    public Long id(){
        return id;
    }
