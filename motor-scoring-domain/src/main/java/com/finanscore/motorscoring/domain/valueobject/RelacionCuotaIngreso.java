    ...
    public RelacionCuotaIngreso {
        ...
        if (porcentaje == null) {
            throw new DomainException(
                "La relación cuota-ingreso es obligatoria."
            );
        }

        if (porcentaje.signum() < 0) {
            throw new DomainException(
                "La relación cuota-ingreso no puede ser negativa."
            );
        }

        porcentaje = porcentaje.setScale(
            4,
            RoundingMode.HALF_UP
        );
    }

    public static RelacionCuotaIngreso calcular(
            BigDecimal montoSolicitado,
            Integer plazoSolicitado,
            BigDecimal ingresosMensuales) {

        if (montoSolicitado == null
                || montoSolicitado.signum() <= 0) {
        ...
        BigDecimal cuotaEstimada = montoSolicitado.divide(
            BigDecimal.valueOf(plazoSolicitado),
            4,
            RoundingMode.HALF_UP
        );

        BigDecimal porcentaje = cuotaEstimada
            .multiply(CIEN)
            .divide(
                ingresosMensuales,
                4,
