...
public record RelacionDeudaIngreso(BigDecimal porcentaje){
    public RelacionDeudaIngreso{
        if(porcentaje==null||porcentaje.signum()<0)throw new DomainException("La relación deuda-ingreso no puede ser negativa.");
    ...
    public static RelacionDeudaIngreso calcular(BigDecimal obligaciones,BigDecimal ingresos){
        if(ingresos==null||ingresos.signum()<=0)throw new DomainException("Los ingresos deben ser mayores que cero.");
