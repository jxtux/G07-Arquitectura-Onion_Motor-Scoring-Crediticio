...
public final class Solicitante{
    ...
    private Solicitante(Long id,NumeroDocumento doc,String nombres,Dinero ingresos,Dinero gastos,Dinero obligaciones,int antiguedad,int activas,int historial,int alertas,EstadoRegistro estado,LocalDateTime fecha){
        ...
        if(nombres==null||nombres.isBlank())throw new DomainException("Los nombres o razón social son obligatorios.");
        ...
        if(ingresos.monto().signum()<=0)throw new DomainException("Los ingresos deben ser mayores que cero.");
        if(ingresos.moneda()!=gastos.moneda()||ingresos.moneda()!=obligaciones.moneda())throw new DomainException("Los datos financieros deben usar la misma moneda.");
        if(antiguedad<0||activas<0||alertas<0)throw new DomainException("Los indicadores no pueden ser negativos.");
        if(historial<0||historial>100)throw new DomainException("El historial de pagos debe estar entre 0 y 100.");
