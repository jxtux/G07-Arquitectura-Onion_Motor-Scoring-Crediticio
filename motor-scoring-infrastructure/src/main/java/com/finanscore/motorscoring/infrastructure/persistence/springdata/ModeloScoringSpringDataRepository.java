...
public interface ModeloScoringSpringDataRepository extends JpaRepository<ModeloScoringJpaEntity,Long>{
    @EntityGraph(attributePaths={
        "versiones","versiones.factores","versiones.factores.reglas"}
        )@Query("select m from ModeloScoringJpaEntity m where m.id=:id")Optional<ModeloScoringJpaEntity> findCompletoById(Long id);
    }
