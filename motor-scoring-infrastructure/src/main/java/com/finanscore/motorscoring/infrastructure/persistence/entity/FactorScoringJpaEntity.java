...
import java.util.*;
@Entity @Table(name="factores_scoring")public class FactorScoringJpaEntity{
    @Id @GeneratedValue(strategy=GenerationType.IDENTITY)@Column(name="id_factor")private Long id;
    @ManyToOne(fetch=FetchType.LAZY)@JoinColumn(name="id_version_modelo",nullable=false)private VersionModeloJpaEntity version;
    ...
    @Column(nullable=false,precision=5,scale=2)private BigDecimal peso;
    @Enumerated(EnumType.STRING)@Column(nullable=false,length=20)private EstadoFactor estado;
    @OneToMany(mappedBy="factor",fetch=FetchType.LAZY)private Set<ReglaEvaluacionJpaEntity> reglas=new HashSet<>();
