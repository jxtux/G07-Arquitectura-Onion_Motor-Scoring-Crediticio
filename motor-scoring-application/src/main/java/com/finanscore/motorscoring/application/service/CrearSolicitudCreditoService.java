...
public final class CrearSolicitudCreditoService implements CrearSolicitudCreditoUseCase{
    ...
    public SolicitudCreditoDto ejecutar(CrearSolicitudCreditoCommand c){
        ...
        if(solicitudes.existePorIdentificadorExterno(ext))throw new SolicitudDuplicadaException("Ya existe una solicitud con identificador externo "+ext.valor());
        ...
        Solicitante solicitante=solicitantes.buscarPorDocumento(doc).map(a->a.actualizarPerfil(c.nombresRazonSocial(),ingresos,gastos,obligaciones,c.antiguedadLaboralNegocio(),c.numeroObligacionesActivas(),c.puntajeHistorialPagos(),c.alertasMora())).orElseGet(()->Solicitante.registrar(doc,c.nombresRazonSocial(),ingresos,gastos,obligaciones,c.antiguedadLaboralNegocio(),c.numeroObligacionesActivas(),c.puntajeHistorialPagos(),c.alertasMora(),ahora));
        solicitante=solicitantes.guardar(solicitante);
        SolicitudCredito solicitud=SolicitudCredito.registrar(solicitante.id(),producto.id(),Dinero.positivo(c.montoSolicitado(),c.moneda()),c.plazoSolicitado(),c.finalidadCredito(),c.canalOrigen(),ext,ahora);
        solicitud=solicitudes.guardar(solicitud);
        return new SolicitudCreditoDto(solicitud.id(),solicitante.id(),ext.valor(),producto.codigo(),solicitud.montoSolicitado().monto(),solicitud.plazoSolicitado(),solicitud.montoSolicitado().moneda().name(),solicitud.estado().name(),solicitud.fechaRegistro());
    }
}
