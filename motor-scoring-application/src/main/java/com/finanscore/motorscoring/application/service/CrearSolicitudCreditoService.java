...
public final class CrearSolicitudCreditoService implements CrearSolicitudCreditoUseCase {
	...
	public CrearSolicitudCreditoService(SolicitanteRepository s, SolicitudCreditoRepository q,
			ProductoCrediticioRepository p, Clock c) {
		solicitantes = s;
		solicitudes = q;
		productos = p;
		clock = c;
	...
	public SolicitudCreditoDto ejecutar(CrearSolicitudCreditoCommand c) {
		...
		ProductoCrediticio producto = productos.buscarPorCodigo(c.codigoProducto())
				.orElseThrow(() -> new RecursoNoEncontradoException("Producto no encontrado: " + c.codigoProducto()));
		NumeroDocumento doc = new NumeroDocumento(c.tipoDocumento(), c.numeroDocumento());
		Dinero ingresos = new Dinero(c.ingresosMensuales(), c.moneda()),
				gastos = new Dinero(c.gastosMensuales(), c.moneda()),
				obligaciones = new Dinero(c.obligacionesFinancieras(), c.moneda());
